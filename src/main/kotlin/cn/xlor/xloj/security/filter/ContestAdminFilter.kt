...
class ContestAdminFilter(
  ...
  override fun doFilter(
    ...
    if (requestURIList.size > 3 && requestURIList[3].toLongOrNull() != null) {
      ...
      if (contest != null) {
        if (userRepository.isUserAdmin(userProfile.id) || userProfile.id == contest.creator || contestRepository.checkUserManageContest(
            contestId,
            userProfile.id
          )
        ) {
          ...
          makeNotFoundResponse(response, "无权访问编号为 $contestId 的比赛")
        }
      } else {
